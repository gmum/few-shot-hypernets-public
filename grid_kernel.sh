...
for HN_KERNEL_LAYERS_NUM in 2 4;
do
  for HN_KERNEL_HIDDEN_DIM in 32 64 128 256;
  do
    for K_TRANSFORMER_LAYERS_NUM in 1;
    do
      for K_TRANSFORMER_HEADS_NUM in 1;
      do
        for K_TRANSFORMER_FF_DIM in 128 256 512 1024;
        do
                ...
                do
                  ...
                  do
                    ...
                    do
                      for LR in 1e-4 1e-3;
                      do
                        SUFFIX="${HN_HIDDEN_SIZE}-${HN_TN_HIDDEN_SIZE}-${HN_TASKSET_SIZE}-${HN_NECK_LEN}-${HN_HEAD_LEN}_${HN_TN_DEPTH}_${LR}-${K_TRANSFORMER_FF_DIM}-${K_TRANSFORMER_HEADS_NUM}-${K_TRANSFORMER_LAYERS_NUM}-${HN_KERNEL_HIDDEN_DIM}-${HN_KERNEL_LAYERS_NUM}_long"

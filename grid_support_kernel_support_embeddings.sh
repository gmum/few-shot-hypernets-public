...
CMD="python train.py --dataset cross_char --num_classes 4112 --method hn_poc_sup_sup_kernel --train_n_way 5 --seed 1 --resume --use_support_embeddings"


for HN_KERNEL_LAYERS_NUM in 4 2 8;
do
  for HN_KERNEL_HIDDEN_DIM in 64 128 256 512;
  do
    for DETACH_EPOCH in 100000 250;
    do
      for HN_HIDDEN_SIZE in 2048 4096 1024 512;
      do
        for HN_TN_HIDDEN_SIZE in 1024 512 256 128;
        do
          ...
          do
            ...
            do
              ...
              do
                for LR in 1e-4;
                do
                  SUFFIX="${HN_HIDDEN_SIZE}-${HN_TN_HIDDEN_SIZE}-${HN_TASKSET_SIZE}-${HN_NECK_LEN}-${HN_HEAD_LEN}_${HN_TN_DEPTH}_${LR}-${HN_KERNEL_HIDDEN_DIM}-${HN_KERNEL_LAYERS_NUM}_sup_sup_kernel_plus_embeddings"

...
def experiment(N):
    ...
    for i in range(model.n_way):
        ...
        fig = plt.figure()
        plt.hist(R1[i], bins=bins, align='left', alpha=0.5, color='darkorange', label='Element from query set')
        plt.hist(R2[i], bins=bins, align='left', alpha=0.5, color='darkgreen', label='Element from support set')
        plt.hist(R3[i], bins=bins, align='left', alpha=0.5, color='cornflowerblue', label='Element out of distribution')
